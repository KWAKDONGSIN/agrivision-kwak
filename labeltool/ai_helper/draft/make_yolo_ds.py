...
OUT = os.path.join(HERE, "..", os.environ.get("DS_NAME", "yolo_ds"))
# V2=1: 포도·복숭아는 원본 정답 번호(포도 CERTH instances_seed · 복숭아 instances_gt + 검수에서 더한 큰 덩어리)
...
for ci, f in enumerate(FR):
    ...
    for stem in stems_of(f):
        ...
        if V2 and f in ("grape", "peach"):
            gp = os.path.join(DATA_DIR, f, "instances_seed" if f == "grape" else "instances_gt", stem + ".png")
            mp = mask_path_of(f, stem)
            if os.path.exists(gp) and os.path.exists(mp):
                from PIL import Image
                g = np.array(Image.open(gp)).astype(np.int32); m = load_mask_bool(mp)
                g[~m] = 0
                left, n = ndimage.label(m & (g == 0))
                if n:
                    sz = ndimage.sum(np.ones_like(left), left, range(1, n + 1))
                    nxt = int(g.max())
                    for k in np.nonzero(sz >= 300)[0]:
                        nxt += 1; g[left == k + 1] = nxt
                arr = g
