# 촬영 단위로 나눈 검증(yolo_ds_v3/val)에서 과일별 마스크 성능과 개수 오차를 잰다 — 결과는 v3_result.txt
import os, glob, sys, numpy as np
from ultralytics import YOLO
m = YOLO("runs/eval_v3/weights/best.pt")
r = m.val(data="../yolo_ds_v3/data.yaml", imgsz=1280, batch=8, plots=False, verbose=False, project="runs", name="eval_v3_val", exist_ok=True)
...
for k in range(0, len(ims), 16):
    ch = ims[k:k + 16]
    for p, rr in zip(ch, m.predict(ch, imgsz=1280, conf=0.25, verbose=False, max_det=1000)):
