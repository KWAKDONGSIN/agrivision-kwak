...
const TOUR_KEY = "tour_seen_260918_paint";
const SW = (c) => '<span class="sw" style="background:' + c + '"></span>';
const KY = (k) => '<span class="key">' + k + "</span>";
const STEPS = [
  ["1. 그림판이라고 생각하세요",
   "<p>사진마다 «열매를 칠한 그림(마스크)» 이 맞는지 보고, 틀렸으면 고치는 툴입니다. 한 장에 보통 10~30초입니다.</p>"
   + "<ul><li><b>왼쪽 = 도구 상자</b> — 붓·지우개·다각형. 맨 위 세 칸에서 <b>무슨 일을 할지</b> 고릅니다"
   + " (🎨 마스크 · ▭ 상자 · ＃ 번호).</li>"
   + "<li><b>가운데 = 사진</b> — 휠로 확대, " + KY("Space") + "+드래그로 이동, " + KY("0") + " 으로 화면 맞춤.</li>"
...
function onTourKey(e) {                     // 등록은 keys.js
  ...
  if (e.key === "?") { e.preventDefault(); tourOpen(); }
