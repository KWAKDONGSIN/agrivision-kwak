...
TOOL_DATA = (os.environ.get("MERGED_TOOL_DATA", "").strip()
...
def build_sandbox():
    ...
    for fruit in FRUITS:
        ...
        put(ks[5], {"status": "ok", "by": "익명", "at": "2026-09-17 12:00", "note": "사람이 봄"},
            dict(kept=True, source="human_unconfirmed", action="keep"))
        put(ks[6], {"status": "exclude", "by": "익명", "at": "2026-09-17 12:01", "note": ""},
            dict(kept=False, source="human_unconfirmed", action="excluded_human", reason="human_exclude"))
        ...
        shutil.copy2(os.path.join(TOOL_DATA, fruit, "duplicates.json"),
...
def t1(p, plan, out_dir):
    ...
    for fruit in FRUITS:
        ...
        # 2026-09-19 종류별 확정: `tool_unconfirmed` 는 «툴 json 은 있으나 사람이 확정하지 않아
        # 채택하지 않았다» 는 **표시**이므로 파일이 없는 것이 맞다(채택된 출처만 센다).
        want = {st for (f2, st), r in man.items()
                if f2 == fruit and r["boxes_source"] not in ("none", "tool_unconfirmed")
                and not r["action"].startswith("excluded")}
        ok_all &= check("T1 %s 상자 = 남긴 사진 것만 · json↔txt 1:1" % fruit,
                        jsons == want == txts, "json=%d want=%d txt=%d" % (len(jsons), len(want), len(txts)))
        # 🔴 2026-09-19 기대값 변경(총괄 결정 · 예전 이름 «툴 상자가 최우선»):
        #   ks[9] 의 툴 json 은 사람이 그렸지만 `confirmed_boxes` 가 **없다**. 확정 없는 초벌
        #   저장본이 정답·추정 상자를 밀어내면 안 되므로 이제 채택하지 않고(여기서는 임성후 판이
        #   채택된다) note 에만 «채택하지 않았습니다» 를 적는다. 확정이 있는 경우는
        #   stage4/test_confirm_kinds.py 가 6×3 조합으로 본다.
        s9 = pl["boxes_kept"][0]
        ok_all &= check("T1 %s 확정 없는 툴 상자는 psm_gt·lsh 를 밀어내지 않는다" % fruit,
                        man[(fruit, s9)]["boxes_source"] in ("psm_gt", "lsh"),
                        man[(fruit, s9)]["boxes_source"])
        ok_all &= check("T1 %s 확정 없는 툴 상자를 note 에 적어 둔다" % fruit,
                        "채택하지 않았습니다" in man[(fruit, s9)]["note"],
                        man[(fruit, s9)]["note"][:100])
    # session·split_group
...
def crosscheck(p, plan, out_dir):
    ...
    if not os.path.exists(os.path.join(copy_root, "export", "export_dataset.py")):
        ...
        # 🔴 2026-09-20 구조 사이클 2 2차 검수: `app/` 이 `core`·`domain`·`api` 묶음으로 쪼개졌다.
        #   맨 위 `.py` 다섯 개만 베끼면 호환 껍데기(`app/maskio.py` → `from domain.maskio import *`)가
        #   `ModuleNotFoundError: No module named 'domain'` 로 죽는다(T5 4과일 전부 실패 · 실측).
        #   그래서 묶음 폴더도 같이 베낀다. 옛 판(묶음 폴더가 없는 코드)에서도 그냥 건너뛴다.
        for pkg in ("core", "domain", "api"):
            src_pkg = os.path.join(TOOL_ROOT, "app", pkg)
            if os.path.isdir(src_pkg):
                shutil.copytree(src_pkg, os.path.join(copy_root, "app", pkg),
                                ignore=shutil.ignore_patterns("__pycache__", "_backup*"),
                                dirs_exist_ok=True)
...
def main():
    ...
    print("\nT7 팀원 폴더가 통째로 없을 때")
    # 2026-09-18 3차 D3: 팀원 폴더가 «직전 빌드에는 있었는데» 사라지면 이제 exit 3 으로 멈춘다.
    # 여기서 보는 것은 «첫 빌드» 기준의 약속(= 견줄 직전 판이 없으면 없어도 멈추지 않는다)이라
    # --no-prev 를 준다. 멈추는 쪽은 stage3/test_fixes_m3.py 의 D3-a 가 본다.
    r7 = run_build(p, "t7", ["--psm-root", os.path.join(SAND, "없는폴더_psm"),
                             "--cih-root", os.path.join(SAND, "없는폴더_cih"),
                             "--lsh-root", os.path.join(SAND, "없는폴더_lsh"), "--no-prev"])
    out7 = os.path.join(p["out"], "datasets_merged_260918_%st7" % RUN)
    check("T7 exit 0 (직전 빌드가 없으면 멈추지 않음)", r7.returncode == 0, r7.stdout[-500:] + r7.stderr[-400:])
    if r7.returncode == 0:
        m7 = read_manifest(os.path.join(out7, "manifest.csv"))
        # 2026-09-19: 팀원 폴더가 없으면 남는 것은 툴 json 뿐인데 그것은 **확정이 없다** →
        # 채택하지 않고 `tool_unconfirmed` 로 표시만 한다(예전 기대값은 `tool`).
        check("T7 상자 출처가 전부 none/tool_unconfirmed",
              {r["boxes_source"] for r in m7.values()} <= {"none", "tool_unconfirmed"},
